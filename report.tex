...
added to a clean dataset, and the change in explained variance ratio and projections between the clean and noisy solutions is recorded. 
...
\begin{tabular}{lll}
\hline
\textbf{Method} & \textbf{Projection distance} & \textbf{Explained variance distance} \\
\hline
Eigendecomposition & 0.0001 & 0.0001  \\
SVD               & 0.0001  & 0.0001  \\
Kernel PCA        & 0.0001  & 0.0001  \\
Randomized PCA    & 0.0001  & 0.0001  \\
Incremental PCA   & 0.0001  & 0.0001  \\
Sparse PCA        & 0.0001  & 0.0001  \\
\hline
\end{tabular}
\caption{Comparison of each PCA method implementation with corresponding Sklearn module.}
...
Scaling experiments are conducted by varying $n$ with fixed $d = 50$, and varying $d$ with fixed $n = 1{,}000$. Runtime and peak memory usage are
...
This report presented a systematic comparison of six PCA implementations across dimensions of correctness, omputational complexity, empirical performance, and  umerical stability. The results confirm that no single method dominates across all settings: the choice of PCA algorithm depends strongly on the structure of the data, the available memory, and the number of components required.

For small to medium datasets with $n \ll d$, Eigendecomposition is straightforward and exact. For large datasets with moderate $d$, SVD offers better numerical stability and similar cost. When only a small number of components are needed from a large dataset, Randomized PCA offers substantial speedups with negligible loss of accuracy. Incremental PCA is the method of choice when data cannot fit in memory or arrives as a stream. Sparse PCA is
appropriate when interpretability of components is important and some loss of explained variance is acceptable. Kernel PCA is uniquely suited to non-linear data but requires careful management of its quadratic memory cost (the improved
algorithm of~\citet{zheng2005improved} can potentially be employed to significantly extends its practical range).

Future work could explore adaptive selection of the kernel hyperparameter $\gamma$ for Kernel PCA, integration of the Nystr\"{o}m approximation as a further memory reduction strategy, and extension of the benchmarking framework to real-world high-dimensional datasets in genomics, image processing, and natural language processing.
...
\textbf{Aurokrupa Sahoo:} PCA by Eigendecomposition, PCA by SVD, Sparse PCA implementation, various analyses (correctness, runtime, memory usage, numerical stability and scaling behaviour). 
\smallskip

\textbf{Anshita Singh:} Kernel PCA implementation, non-linear data generation.
\smallskip 

\textbf{Debagnik Das:} Randomized PCA implementation.
\smallskip

\textbf{Sadekar Suchet Samir:} Incremental PCA implementation, correlated, uncorrelated, and low-rank data generation.
\smallskip 
...
\url{https://github.com/Suchet17/PCA_Methods_Comparison.git}.
...
\begin{figure}[tbph!]
    \begin{center}
        \includegraphics[width=\textwidth]{figs/kpca/basic_implementation.png}
        \caption{Implementing Kernel PCA and PCA for non-linear data.}
...
\begin{algorithm}[H]
\caption{Kernel PCA}\label{alg:kpca}
\begin{algorithmic}
